...
static const size_t MAX_FALLBACK_SIZE = 4096; //(size_t) atoi(optional_ptr(getenv("UWS_HTTP_MAX_HEADERS_SIZE")).value_or((char *) "4096"));
            ...
            * ought to be handled as an error. */
            ...
            if (transferEncodingString.length()) {
                ...
                if (!CONSUME_MINIMALLY) {
                    ...
                    unsigned int cnsumed = (length - (unsigned int) dataToConsume.length());
                    data = (char *) dataToConsume.data();
                    length = (unsigned int) dataToConsume.length();
                    consumedTotal += cnsumed;
                }
