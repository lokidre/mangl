  ...
  try {
    ...
    for(;;) {
      ...
      try {
        ...
        int client_socket = ::accept ( server_socket, (sockaddr *)&conn_addr, &addr_len ) ;
        ...
        {
          cout << "Connection established: " << inet_ntoa(conn_addr.sin_addr) << endl ;
        }
        ...
        SSL_set_fd(ssl, client_socket);    // set connection to SSL state
        if ( SSL_accept(ssl) <= 0 )           // start the handshaking
          openssl_raise_error() ;
