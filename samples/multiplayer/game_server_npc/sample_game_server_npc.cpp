...
#include "../shared/sample_game_npc.hpp"
...
static void start() 
{
    ...
    gameServer_->onDebugMessage = [](auto text) { Console::log(text); };
    ...
    gameServer_->onGameNpcCreate = [] {
        return new SampleGameNpc();
    };

    gameServer_->onGameNpcDestroy = [](auto npc) {
        delete npc;
    };
    ...
    gameClient_->onMatch = [](auto packet) {
        ...
        if (packet->isMatchCountdown()) Console::log("Countdown: %s", packet->value);
        if (packet->isMatchBegin()) Console::log("Match Begin!");
        if (packet->isMatchPaused()) Console::log("Match Paused.");
        if (packet->isMatchResumed()) Console::log("Match Resumed.");
    };

    gameClient_->onGame = [](auto packet) {
        if (packet->isGameState()) { Console::log("Game State."); return; }
    };
