...
static net::BluetoothClient* connection_{};
...
static void start() {
    ...
    auto connection = new net::BluetoothClient();

    connection->onError = [](PeerKey pk, ErrorInfo err) {
        println("Error: %s", err.text);
        finishFlag_ = true;
    };

    connection->onPeerFound = [connection, di](auto pk, auto pdi) {
        connection->stopDiscovery();

        println("Discovered: %s", pdi.build());

        hostUrl_ = pdi.getUrl();
        connection->connect(hostUrl_, di);
    };

    connection->onPeerState = [](PeerKey, PeerState ps) {
        println("Connection Status: %c", M::getPeerStateString(ps));
    ...
    connection->onData = [](PeerKey, DataParamA data) {
        if (checkPacket(currentPacket_, data)) {
            packetSent_ = false;
            println("Packets are matching");
            packageTimestamp = timestamp + 1;
        } else {
            println("Packet Error: Packets don't match");
            finishFlag_ = true;
        }
    };
        ...
        if (timestamp >= packageTimestamp) {
            if (connected_) {
                ...
                if (!packetSent_) {
                    ...
                    println("Sending packet: %d", currentPacket_.size());
