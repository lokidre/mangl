...
inline void pack(const PodObject& obj, apl::JsonBuilder* builder);
inline void unpack(const apl::Json& packet, PodObject* obj);

inline auto packStringJson(const PodObject& obj) {
    apl::JsonBuilder builder;
    pack(obj, &builder);
    return builder.str();
}

inline auto unpackStringJson(std::string_view str, PodObject* obj) {
    apl::Json packet;
...
inline auto packFileJson(std::string_view filename, const PodObject& obj) {
    auto str = packStringJson(obj);
    apl::saveFile(filename, str);
}

inline auto unpackFileJson(std::string_view filename, PodObject* obj) {
...
inline void packObject(const PodObject& obj, std::string_view name, apl::JsonBuilder* builder);
inline void unpackObject(const apl::JsonNode& node, PodObject* obj);
...
inline void pack(const PodObject& obj, apl::JsonBuilder* builder) {
    builder->begin();
    impl_PodObject_json::packObject(obj, "PodObject", builder);
    builder->end();
}

inline void unpack(const apl::Json& root, PodObject* obj) {
    impl_PodObject_json::clearObject(obj);
    for (auto& docNode: root.front()) {
        impl_PodObject_json::unpackObject(docNode, obj);
    }
}
